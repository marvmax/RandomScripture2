...
struct StandardWorks {
  ...
  func getBookInt(standardWork work: String) -> Int {
    ...
    var acsess = 0
    //var count: Int = -1
    //Check for Standard Works
    /*if work == standardWorks[0] {
      acsess = Int.random(in: 0...4)
    } else {
      // Check for the individual books
    ...
    switch work {
    case "Standard Works":
      acsess = Int.random(in: 3...7)
    case "Bible":
      acsess = Int.random(in: 3...4)
    case "Triple Combination":
      acsess = Int.random(in: 5...7)
    case "Old Testament":
      acsess = 3
    case "New Testament":
      acsess = 4
  ...
  func getChapter(arrayAcess: Int) -> String {
    // Used for testing
    // print("Entering PickBook.getChapter")
    if arrayAcess != -3 {
